...
cv::Mat genLbp(const cv::Mat &oImgSrc, int nLbpOp = 1, int nRad = 1, int nNbrNum = 8)
{
	...
	case 0:
		...
		{
		case CV_8SC1:
			lbp::ELBP_<char>(oImgDst, oImgLbp);
			break;
		case CV_8UC1:
			lbp::ELBP_<unsigned char>(oImgDst, oImgLbp);
			break;
		case CV_16SC1:
			lbp::ELBP_<short>(oImgDst, oImgLbp);
			break;
		case CV_16UC1:
			lbp::ELBP_<unsigned short>(oImgDst, oImgLbp);
			break;
		case CV_32SC1:
			lbp::ELBP_<int>(oImgDst, oImgLbp);
			break;
		case CV_32FC1:
			lbp::ELBP_<float>(oImgDst, oImgLbp);
			break;
		case CV_64FC1:
			lbp::ELBP_<double>(oImgDst, oImgLbp);
			break;
		}
		break;
	case 1:
		...
		{
		case CV_8SC1:
			lbp::OLBP_<char>(oImgDst, oImgLbp);
			break;
		case CV_8UC1:
			lbp::OLBP_<unsigned char>(oImgDst, oImgLbp);
			break;
		case CV_16SC1:
			lbp::OLBP_<short>(oImgDst, oImgLbp);
			break;
		case CV_16UC1:
			lbp::OLBP_<unsigned short>(oImgDst, oImgLbp);
			break;
		case CV_32SC1:
			lbp::OLBP_<int>(oImgDst, oImgLbp);
			break;
		case CV_32FC1:
			lbp::OLBP_<float>(oImgDst, oImgLbp);
			break;
		case CV_64FC1:
			lbp::OLBP_<double>(oImgDst, oImgLbp);
			break;
		}
		break;
	case 2:
		...
		{
		case CV_8SC1:
			lbp::VARLBP_<char>(oImgDst, oImgLbp);
			break;
		case CV_8UC1:
			lbp::VARLBP_<unsigned char>(oImgDst, oImgLbp);
			break;
		case CV_16SC1:
			lbp::VARLBP_<short>(oImgDst, oImgLbp);
			break;
		case CV_16UC1:
			lbp::VARLBP_<unsigned short>(oImgDst, oImgLbp);
			break;
		case CV_32SC1:
			lbp::VARLBP_<int>(oImgDst, oImgLbp);
			break;
		case CV_32FC1:
			lbp::VARLBP_<float>(oImgDst, oImgLbp);
			break;
		case CV_64FC1:
			lbp::VARLBP_<double>(oImgDst, oImgLbp);
			break;
		}
		break;
	}
...
float compAppMdl(char *acInFeatFlrPthPrb, int nIdPrb, char *acInFeatFlrPthGall, int nIdGall, double fDistThld, int nHistCmpTyp, int nEmdDistTyp = CV_DIST_L1)
{
	...
	std::sprintf(acInFeatPth, "%s", acInFeatFlrPthPrb);
	...
	std::sprintf(acInFeatPth, "%s", acInFeatFlrPthGall);
	std::strcat(acInFeatPth, acInFeatNm);
	ifsFeat.open(acInFeatPth);
	ifsFeat.getline(acInFeatBuf, 2048);
	while (!ifsFeat.eof())
	...
	for (unsigned int i = 0; i < vvfAppMdlPrb.size(); i++)
	{
		for (unsigned int j = 0; j < vvfAppMdlGall.size(); j++)
		{
			for (int b = 0; b < 88; b++)
			{
...
{
	char acInTrkTxt[256] = {};
	std::sprintf(acInTrkTxt, "%s", argv[1]);
	char acInFrmFlrPth[256] = {};
	std::sprintf(acInFrmFlrPth, "%s", argv[2]);
	char acOutFlrPth[256] = {};
	std::sprintf(acOutFlrPth, "%s", argv[3]);
	...
	float fDetScr, fDep;
	double fRoofRiSq = (double)((APP_MDL_NORM_SZ.width * APP_MDL_NORM_SZ.width) + (APP_MDL_NORM_SZ.height * APP_MDL_NORM_SZ.height)) / 4.0;
	double fProb;
			...
			for (std::vector<CTrkNd>::iterator it = voTrkNd.begin(); it != voTrkNd.end(); ++it)
			{
				...
				if (!it->getFrmBrdrFlg())
				{
					...
					if (APP_MDL_VIS_THLD < ((double)nMskArea / (double)nMskInitArea))
					{
						...
						std::sprintf(acOutFeatPth, "%s", acOutFlrPth);
						...
						while (vfIdMinDep.size() <= (unsigned int)nId)
						...
						if ((APP_MDL_SMP_NUM > nSmpCnt) || ((APP_MDL_SMP_NUM <= nSmpCnt) && (fProb <= (vfIdMinDep[nId] / it->getDep()))))
						{
							...
							poFeatFl = std::fopen(acOutFeatPth, "w");
							for (unsigned int i = 0; i < vvfAppMdl.size(); i++)
							{
